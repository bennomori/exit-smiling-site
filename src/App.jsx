...
import { getMemberBioParagraphs, mergeMemberBioMedia } from "./memberBioMedia";
...
const tourDates = [
  ...
  },
  {
    ...
    date: 'JUN 26',
    city: 'Moruya, NSW',
    venue: 'Winter SOULstice - Sage Gardens Moruya',
    time: '5PM-8PM AEST',
    href: 'https://www.eventbrite.com/e/winter-soulstice-2026-tickets-1988807642195?aff=ebdssbdestsearch',
    mapHref: 'https://maps.app.goo.gl/TDEzi6o7wiJLemxr5',
    ctaLabel: 'Tickets',
    note: 'Winter SOULstice 2026',
  },
];
      ...
      <div className="mx-auto max-w-7xl px-6 py-20">
        ...
        <div className="mt-10 divide-y divide-white/10 rounded-3xl border border-white/10">
          {visibleTourDates.map((show, index) => {
            ...
            >
              <div
                className={`pointer-events-none absolute inset-0 rounded-[inherit] border transition duration-300 ${isUpcoming ? 'gig-upcoming-pulse border-yellow-200/35 shadow-[0_0_42px_rgba(250,204,21,0.18),inset_0_0_28px_rgba(250,204,21,0.06)] group-hover:border-yellow-100/70 group-hover:shadow-[0_0_62px_rgba(250,204,21,0.28),inset_0_0_34px_rgba(250,204,21,0.09)]' : 'border-transparent opacity-0 group-hover:border-white/30 group-hover:opacity-100'}`}
                style={isUpcoming ? { animationDelay: `${pulseDelaySeconds}s`, animationDuration: `${pulseDurationSeconds}s` } : undefined}
              />
              {show.posterImage ? (
                index < 3 ? (
                  <button
...
function MemberCard({ member, memberMediaOverrides }) {
  ...
  const memberBioParagraphs = useMemo(
    () => getMemberBioParagraphs(member.name, memberMediaOverrides),
    [member.name, memberMediaOverrides]
  );
  ...
  const livePreview =
    member.name === 'Cadence'
      ? {
          src: 'https://exit-smiling-media.bennoclark.workers.dev/bio/cadence/cadence-top-bio-video-20s.mp4',
  ...
  const getBioImageCropY = (image) => {
    ...
    if (member.name === "Lando" || image?.orientation === "portrait") return 0;
    return 50;
  };
  ...
  return (
    <>
      ...
      {memberBioParagraphs.length ? (
        memberBioParagraphs.map((paragraph, index) => (
          <p key={`${member.name}-bio-${index}`} className="mt-3 text-sm text-white/70">
            {paragraph}
          </p>
        ))
      ) : (
